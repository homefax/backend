...
export class BlockchainController {
  ...
  async createReport(
    ...
    try {
      const { propertyId, reportType, reportHash, price } = reportData;
      const userAddress = req.user.walletAddress;
  ...
  async getReport(@Request() req, @Param('id') reportId: number) {
    try {
      ...
      throw new BadRequestException(
        `Failed to get report: ${error.message}`,
      );
    }
  }
  ...
  @ApiResponse({ status: 200, description: 'Report content retrieved successfully' })
  @ApiResponse({ status: 400, description: 'Bad request' })
